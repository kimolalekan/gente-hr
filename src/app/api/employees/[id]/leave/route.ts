...
/** GET /api/employees/[id]/leave — leave history for the employee. */
export const GET = route(
  async (_request: Request, { params }: { params: Promise<{ id: string }> }) => {
    ...
    const { db, pool } = await getDb();
    const { leaves } = await import("@db/schema");
    try {
      const rows = await db
        .select()
        .from(leaves)
        .where(
          and(eq(leaves.tenantId, user.tenantId), eq(leaves.employeeId, id)),
        )
